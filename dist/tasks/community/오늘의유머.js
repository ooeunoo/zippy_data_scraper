...
let 오늘의유머 = class 오늘의유머 extends page_task_1.PageTask {
    ...
    async getContentUrls(page, baseUrl) {
        const urls = await page.evaluate(async (baseUrl) => {
            const table = document.querySelector('.table_list > tbody');
            const contents = table.querySelectorAll('.view');
            const urls = [];
            for (const content of contents) {
                const titleSection = content.querySelector('.subject');
                if (titleSection) {
    ...
    async getTitle(page) {
        return page.evaluate(async () => {
            const _title = document.querySelector('div.viewSubjectDiv');
            return _title === null || _title === void 0 ? void 0 : _title.textContent.trim();
        });
    }
    async getAuthor(page) {
        return page.evaluate(async () => {
            const _author = document.querySelector('#viewPageWriterNameSpan');
    ...
    async getCreatedAt(page) {
        const createdAt = await page.evaluate(async () => {
            const _createdAt = document.querySelector('#containerInner > div.writerInfoContainer > div.writerInfoContents > div:nth-child(7)');
            return _createdAt
                ? _createdAt.textContent
                    .replace('등록시간 : ', '')
                    .replace(/\//g, '-')
                    .trim()
                : null;
    ...
    async getContentImageUrl(page) {
        const contentImgUrl = await page.evaluate(async () => {
            var _a, _b;
            const _1 = document.querySelector('.viewContent');
            const _contentImgUrl = _1 === null || _1 === void 0 ? void 0 : _1.querySelectorAll('img');
