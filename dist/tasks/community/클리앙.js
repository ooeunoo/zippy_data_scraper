...
const task = task_constant_1.TASK_MAP.클리앙;
let 클리앙 = class 클리앙 extends page_task_1.PageTask {
    ...
    async getContentUrls(page, baseUrl) {
        const urls = await page.evaluate(async (baseUrl) => {
            ...
            for (const content of contents) {
                const urlElement = content.querySelector('div.list_title > a.list_subject');
                if (urlElement) {
        ...
        return urls.map((url) => (0, url_1.removeParamUrl)(url, ['od', 'po', 'category', 'groupCd']));
    }
    async getTitle(page) {
    ...
    async getAuthor(page) {
        return page.evaluate(async () => {
            const author = document.querySelector('div.post_info > div.post_contact > span.contact_name > span.nickname');
    ...
    async getCreatedAt(page) {
        return page.evaluate(async () => {
            ...
            const postAuthorText = _1 === null || _1 === void 0 ? void 0 : _1.textContent;
            const match = postAuthorText === null || postAuthorText === void 0 ? void 0 : postAuthorText.match(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
            const createdAt = match ? match[0] : null;
            return createdAt === null || createdAt === void 0 ? void 0 : createdAt.trim();
        });
    }
...
exports.클리앙 = 클리앙;
//# sourceMappingURL=%ED%81%B4%EB%A6%AC%EC%95%99.js.map