...
const task = TASK_MAP.오늘의유머;

@Injectable()
export class 오늘의유머 extends PageTask {
  ...
  async getAuthor(page: Page): Promise<string> {
    return page.evaluate(async () => {
      ...
      return _author?.textContent.replace('★', '').trim();
    });
  }
  ...
  async getCreatedAt(page: Page): Promise<string> {
    const createdAt = await page.evaluate(async () => {
      const _createdAt = document.querySelector(
        '#containerInner > div.writerInfoContainer > div.writerInfoContents > div:nth-child(7)',
      );
      return _createdAt
